...
export function ProductCard({ product }: { product: Product }) {
  ...
  const displayPrice =
    p.category === "panel" && p.wattPeak && p.pricePerWatt
      ? `${formatVnd(Math.round(p.wattPeak * p.pricePerWatt), locale)} ${t("perPanel")}`
      : p.price
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <h3 className="mt-1 text-lg font-bold text-navy-950">{p.name}</h3>
